...
// Tests for messages unchanged from eth/70 are covered by the main eth suite.

const (
	eth71ProtocolVersion = 71

	eth71GetBlockAccessListsMsg = 0x12
	eth71BlockAccessListsMsg    = 0x13
)

type eth71GetBlockAccessListsPacket struct {
	RequestId uint64
	Hashes    []common.Hash
}

type eth71BlockAccessListsPacket struct {
	RequestId   uint64
	AccessLists rlp.RawList[rlp.RawValue]
}
...
func (s *Suite) eth71GetBlockAccessLists(t *utesting.T, tc *accessListsTest) error {
	...
	req := &eth71GetBlockAccessListsPacket{
		RequestId: uint64(rand.Int63()),
		Hashes:    tc.hashes,
	}
	if err := conn.Write(ethProto, eth71GetBlockAccessListsMsg, req); err != nil {
		return fmt.Errorf("access list request failed: %v", err)
	}
	var res eth71BlockAccessListsPacket
	if err := conn.ReadMsg(ethProto, eth71BlockAccessListsMsg, &res); err != nil {
		return fmt.Errorf("access list response failed: %v", err)
	}
	return s.validateAccessListsResponse(t, tc, req.RequestId, res.RequestId, res.AccessLists)
}
